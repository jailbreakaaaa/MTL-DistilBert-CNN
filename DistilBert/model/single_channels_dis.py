...
class MultiTaskLossWrapper(nn.Module):
    def __init__(self, model, discriminator, optimizer_d, device="cuda"):
    ...
    def forward(self, input, targets):
        ...
        task_loss = F.cross_entropy(outputs[1], targets[1]) * 0.15
        # dis_loss = F.cross_entropy(self.discriminator(outputs[2].detach()), targets[1]) * -1  # 这个有点大, 会导致loss变为负数
        # self.optimizer_d.zero_grad()
        # dis_loss.backward()
        # self.optimizer_d.step()

        dis_loss = F.cross_entropy(self.discriminator(outputs[2]), targets[1]) * -0.05

        result = {"target": outputs[0].argmax(dim=1).tolist(), "task": outputs[1].argmax(dim=1).tolist()}
        return result, {"target": target_loss, "task": task_loss, "dis_loss": dis_loss, "total": target_loss+task_loss+dis_loss}
               # weight.tolist()
...
class generate(nn.Module):
    ...
    def forward(self, input):
        ...
        task = self.fc_task(fusion_layer)

        # discriminator_d = self.fc_discriminator(share_layer)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, dim, num_tasks):
        ...
        self.linear = nn.Linear(dim, num_tasks)       #2048*2,2048*2
        # self.softmax = nn.Sequential(OrderedDict([
        #     ('linear', nn.Linear(1024, num_tasks)),
        #     ('log_softmax', nn.LogSoftmax(dim=1))           #区分属于哪个任务
        # ]))
