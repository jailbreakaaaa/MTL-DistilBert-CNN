...
class get_embedding(nn.Module):
    ...
    def __init__(self, args, requires_grad):
...
class generate(nn.Module):
    def __init__(self, args):
        ...
        # self.args = args
        self.embedding0 = get_embedding(args, requires_grad=False)
        self.embedding1 = get_embedding(args, requires_grad=True)

        self.share_layer0 = CNN_layers(args.num_channels, args.kernel_sizes, args.glove_dim*2)
        self.share_layer1 = TransformerModel(args.glove_dim*2, args.nhead, args.nhid, args.nlayers, dropout=args.dropout_trans)
        self.private_layer = nn.ModuleList([GRU_attn(args.glove_dim*2, args.enc_hid_size, args.rnn_layers,
                                                     args.bidirectional, args.dec_hid_size,
                                                     args.dropout_rnn)]*args.task_num)  # 多任务对应的decoder
        self.fushion = nn.Linear(sum(args.num_channels)+args.glove_dim*2, args.enc_hid_size*2)
    ...
    def forward(self, input):
        ...
        emb0 = self.embedding0(input["x"])
        emb1 = self.embedding1(input["x"])
        emb = torch.cat((emb0, emb1), dim=-1)
        share_layer = self.share_layer0(emb.permute(0, 2, 1))  # (batch_size, batch_size)
        ...
        task = self.fc_task(torch.cat((private_layer, share_layer), dim=1))
