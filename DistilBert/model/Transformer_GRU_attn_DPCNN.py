...
class generate(nn.Module):
    def __init__(self, args):
        ...
        self.dpcnn = DPCNN(args.num_filters, args.hidden_size, args.out_size)
        ...
        if args.bidirectional:
            self.fc_target = nn.Linear(args.out_size, args.output_size)
            self.fc_task = nn.Linear(args.out_size, args.task_num)
            nn.init.xavier_uniform(self.fc_target.weight)
            nn.init.xavier_uniform(self.fc_task.weight)
    ...
    def forward(self, input):
        ...
        # share_layer = self.dropout(self.share_layer2(emb.permute(0, 2, 1)))  # (batch_size, batch_size)
        share_layer = self.dropout(self.share_layer1(emb))  # transformer的输出比较大, 300维度
        ...
        out_layer = self.dpcnn(torch.cat((private_layer, share_layer), dim=-1))
        # fusion_layer = torch.cat((share_layer, private_layer), dim=1)
        target = self.fc_target(out_layer)
        task = self.fc_task(out_layer)
