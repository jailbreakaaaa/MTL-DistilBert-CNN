...
class generate(nn.Module):
    def __init__(self, args):
        ...
        if args.bidirectional:
            self.fc_target = nn.Linear(sum(args.num_channels)+args.glove_dim+args.enc_hid_size*2, args.output_size)
            self.fc_task = nn.Linear(sum(args.num_channels)+args.glove_dim+args.enc_hid_size*2, args.task_num)
    ...
    def forward(self, input):
        ...
        emb = self.dropout(self.embedding0(input["x"]))
        # emb = F.gelu(self.fc(emb))
        share_layer0 = self.share_layer0(emb.permute(0, 2, 1))  # (batch_size, batch_size)
        share_layer1 = self.share_layer1(emb)
        share_layer = torch.cat((share_layer0, share_layer1), dim=-1)
