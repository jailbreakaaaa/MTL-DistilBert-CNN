...
class generate(nn.Module):
    def __init__(self, args):
        ...
        self.private_layer1 = GRU_attn(args.glove_dim, args.enc_hid_size, args.rnn_layers,
                                                     ...
                                                     args.dropout_rnn)  # 多任务对应的decoder
        self.private_layer2 = GRU_attn(args.glove_dim, args.enc_hid_size, args.rnn_layers,
                                                     args.bidirectional, args.dec_hid_size,
                                                     args.dropout_rnn)  # 多任务对应的decoder
        ...
        if args.bidirectional:
            ...
            self.fc_target = nn.Linear(args.enc_hid_size*2+sum(args.num_channels)+args.glove_dim, args.output_size)
            self.fc_task = nn.Linear(args.enc_hid_size*2+sum(args.num_channels)+args.glove_dim, args.task_num)
        else:
    ...
    def forward(self, input):
        ...
        share_layer = self.dropout(torch.cat((share_layer0, share_layer1), dim=1))
        # emb, seq_len
        private_layer1 = self.dropout(self.private_layer1(emb, input["seq_len"]))  # (batch_size, num_channels*3)
        private_layer2 = self.dropout(self.private_layer2(emb, input["seq_len"]))
        # private_layer = self.dropout(torch.cat((private_layer1)))
        # fusion_layer = torch.cat((share_layer, private_layer), dim=1)
        target = self.fc_target(torch.cat((share_layer, private_layer1), dim=1))
        task = self.fc_task(torch.cat((share_layer, private_layer2), dim=1))
