...
class GRU_attn(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        s = torch.tanh(self.dropout(self.fc(torch.cat((sent_hidden[-2, :, :], sent_hidden[-1, :, :]), dim=1))))
...
class generate(nn.Module):
    def __init__(self, args):
        ...
        # transformer的输出维度是 glove_dim, cnn的输出是 sum(num_channels)
        ...
        # GRU的输出大小 enc_hid_size * 2
        self.fushion = nn.Linear(sum(args.num_channels)+args.glove_dim, args.enc_hid_size)
        if args.bidirectional:
            ...
            self.fc_target = nn.Linear(args.enc_hid_size*3, args.output_size)
            self.fc_task = nn.Linear(args.enc_hid_size*3, args.task_num)
        else:
    ...
    def forward(self, input):
        ...
        share_layer = self.dropout(F.gelu(self.fushion(torch.cat((share_layer0, share_layer1), dim=1))))
        # emb, seq_len
        ...
        target = self.dropout(self.fc_target(torch.cat((share_layer, private_layer), dim=1)))
        task = self.dropout(self.fc_task(torch.cat((share_layer, private_layer), dim=1)))
