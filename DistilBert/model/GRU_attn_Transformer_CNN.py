...
class GRU_attn(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        # 原始 # local_representation = self.pool(attn_weights.matmul(sent_output)).squeeze(-1)  # batch_size, batch_size, enc_hid_dim
        # batch, enc_hid_size * 2
        local_representation = torch.bmm(sent_output.transpose(1, 2), attn_weights.unsqueeze(2)).squeeze(-1)
        # 不使用attention
        # local_representation = self.fc(sent_output) 未完成
